...
export default function VerifyQuote() {
  ...
  // --- NUEVA FUNCIÓN: IMPRIMIR COTIZACIÓN ---
  ...
  const handleDownloadAllSpecs = () => {
    ...
    const urls = quote.items
      .map(item => item.technical_spec_url)
  ...
  const hasTechSpecs = quote.items?.some(i => i.technical_spec_url);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-200 selection:bg-cyan-500/30 font-sans print:bg-white print:text-black">
      {/* Background Decor (Oculto en impresión) */}
      ...
      <div className="relative max-w-3xl mx-auto px-6 py-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8 print:hidden">
          {/* BOTÓN DESCARGAR COTIZACIÓN (SOLUCIÓN) */}
          <button 
            onClick={handlePrintQuote}
            className="bg-orange-600 hover:bg-orange-500 text-white font-black py-4 rounded-2xl shadow-xl shadow-orange-950/40 flex items-center justify-center gap-3 transition-all active:scale-[0.98] border border-orange-400/20"
          >
            <Printer className="w-5 h-5" />
            <span className="text-sm tracking-tight uppercase">Imprimir / Guardar PDF Oferta</span>
          </button>

          {/* BOTÓN DESCARGAR FICHAS */}
          {hasTechSpecs && (
            <button 
        ...
        {/* Main Certificate Card (Este es el que se imprime) */}
        <div className="bg-slate-900/50 backdrop-blur-xl border border-slate-800 rounded-3xl overflow-hidden shadow-2xl mb-8 print:bg-white print:border-none print:shadow-none print:text-black">
          ...
          <div className="p-8">
            <h3 className="text-slate-400 font-black text-[10px] uppercase tracking-[0.2em] mb-6 flex items-center gap-2 print:text-black">
              <FileText className="w-4 h-4 text-cyan-500 print:hidden" />
              ...
              {quote.items?.map((item, idx) => (
                <div key={idx} className="group relative bg-slate-950/40 border border-slate-800/50 p-4 rounded-2xl print:bg-white print:border-black print:text-black">
                  <div className="flex items-center justify-between gap-4">
                    ...
                    {/* Indicador de ficha técnica (solo visual en la web) */}
            ...
            {/* TOTALES (Solo visibles al imprimir o ver certificado) */}
            <div className="mt-8 pt-6 border-t border-slate-800 print:border-black">
               <div className="flex justify-between items-center">
                 ...
                 <span className="text-white font-mono font-bold text-xl print:text-black">
                   ${Number(quote.total || 0).toLocaleString('es-CL')}
                 </span>
               </div>
            </div>
          </div>
        ...
        <div className="text-center px-4">
          ...
          <p className="text-slate-500 text-[11px] leading-relaxed italic print:text-black">
            "Este certificado garantiza que la oferta comercial adjunta ha sido generada bajo los estándares de calidad de Comtec Industrial. La integridad técnica de los ítems aquí listados corresponde a los registros vigentes en nuestra base de datos."
          </p>
        </div>
      </div>
